...
class Asteroid(Wrapper):
    ...
    LARGE = 3
    life123123=0
    images = {SMALL  : games.load_image("taikong.bmp"),
    ...
    def die(self):
        ...
        if self.size==1:
            ...
            super(Asteroid, self).die()
# class Asteroid123(Wrapper):
#     """ An asteroid which floats across the screen. """
#     SMALL = 1
#     MEDIUM = 2
#     LARGE = 3
#     images = {SMALL : games.load_image("taikong.bmp"),
#               MEDIUM : games.load_image("taikong.bmp"),
#               LARGE : games.load_image("taikong.bmp") }
...
#         super(Asteroid123, self).__init__(
#             image = Asteroid123.images[size],
#             x = x, y = y,
#             dx = random.choice([1, -1]) * Asteroid123.SPEED * random.random()/size, 
#             dy = random.choice([1, -1]) * Asteroid123.SPEED * random.random()/size)
...
#         self.game.score.value += int(Asteroid123.POINTS / self.size)
#         self.game.score.right = games.screen.width - 10   
        
#         if asteroid isn't small, replace with two smaller asteroids
#         if self.size != Asteroid123.SMALL:
#             for i in range(Asteroid123.SPAWN):
#                 new_asteroid = Asteroid123(game = self.game,
#                                         x = self.x,
#                                         y = self.y,
#                                         size = self.size - 1)
#                 games.screen.add(new_asteroid)
...
#         if all asteroids are gone, advance to next level    
#         if Asteroid123.total == 0:5
#            self.game.advance()

#        super(Asteroid123, self).die()
class Ship(Collider):
    """ The player's ship. """
    ...
    def advance(self):
        ...
        for i in range(self.level):
            ...
            new_asteroid = Asteroid(game = self,
                                    ...
                                    size = Asteroid.LARGE)
            games.screen.add(new_asteroid)
            # new_asteroid1 = Asteroid123(game = self,
            #                         x = x, y = y,
            #                         size = Asteroid123.LARGE)
            # games.screen.add(new_asteroid1)
